    ...
    for frame in 0..num_frames {
      ...
      for ch in 0..2 {
        ...
        if self.sin_scale != 0.0 {
          distorted += (normalized * self.sin_scale as f64 + self.sin_offset as f64).sin();
        }

        if self.cos_scale != 0.0 {
          distorted += (normalized * self.cos_scale as f64 + self.cos_offset as f64).cos();
        }

        if self.tan_scale != 0.0 {
          let tan_input = (normalized * self.tan_scale as f64 + self.tan_offset as f64).clamp(

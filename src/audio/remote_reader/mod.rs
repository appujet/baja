...
impl BaseRemoteReader {
  pub fn new(client: reqwest::blocking::Client, url: &str) -> AnyResult<Self> {
    ...
    info!("Opened BaseRemoteReader: {} (len={:?})", url, len);
...
impl Read for BaseRemoteReader {
...
impl Seek for BaseRemoteReader {
  fn seek(&mut self, pos: SeekFrom) -> std::io::Result<u64> {
    ...
    if new_pos != self.pos {
      ...
      if forward_jump > 0 && forward_jump <= (buf_remaining + next_buf_remaining) {
        debug!(
          "BaseRemoteReader purely in-memory seek forward by {} bytes",
          forward_jump
        );
      ...
      debug!("BaseRemoteReader hard seek: {} -> {}", self.pos, new_pos);
      self.buf.clear();
      self.buf_pos = 0;
      self.pos = new_pos;
...
impl MediaSource for BaseRemoteReader {

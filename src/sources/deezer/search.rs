  ...
  pub(crate) async fn get_autocomplete(
    ...
    if all_types || types.contains(&"album".to_string()) {
      if let Some(data) = json.pointer("/albums/data").and_then(|v| v.as_array()) {
        for album in data {
          let title = album.get("title").and_then(|v| v.as_str()).unwrap_or("Unknown Album").to_string();
          let link = album.get("link").and_then(|v| v.as_str()).unwrap_or("").to_string();
          let cover_xl = album.get("cover_xl").and_then(|v| v.as_str()).unwrap_or("").to_string();
          let artist_name = album.pointer("/artist/name").and_then(|v| v.as_str()).unwrap_or("Unknown Artist").to_string();
    ...
    if all_types || types.contains(&"artist".to_string()) {
      if let Some(data) = json.pointer("/artists/data").and_then(|v| v.as_array()) {
        for artist in data {
          let name = artist.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown Artist").to_string();
          let link = artist.get("link").and_then(|v| v.as_str()).unwrap_or("").to_string();
          let picture_xl = artist.get("picture_xl").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    if all_types || types.contains(&"playlist".to_string()) {
      if let Some(data) = json.pointer("/playlists/data").and_then(|v| v.as_array()) {
        for playlist in data {
          let title = playlist.get("title").and_then(|v| v.as_str()).unwrap_or("Unknown Playlist").to_string();
          let link = playlist.get("link").and_then(|v| v.as_str()).unwrap_or("").to_string();
          let picture_xl = playlist.get("picture_xl").and_then(|v| v.as_str()).unwrap_or("").to_string();
          let creator_name = playlist.pointer("/creator/name").and_then(|v| v.as_str()).unwrap_or("Unknown Creator").to_string();
          let nb_tracks = playlist.get("nb_tracks").and_then(|v| v.as_u64()).unwrap_or(0);

...
/// Decrypt an encrypted Gaana stream path to produce an HLS URL.
///
/// The stream API returns an encrypted string where:
/// 1. The first character is a digit offset
/// 2. Skip `offset + 16` characters from the start
/// 3. Remaining is base64-encoded AES-128-CBC ciphertext (NoPadding)
/// 4. Decrypted text contains an HLS path starting with `hls/`
pub fn decrypt_stream_path(encrypted_data: &str) -> Option<String> {
    if encrypted_data.is_empty() {
        return None;
    }

    let offset = encrypted_data
        .chars()
        .next()?
        .to_digit(10)? as usize;

    let skip = offset + 16;
    if skip >= encrypted_data.len() {
        warn!("Gaana: encrypted data too short (len={}, skip={})", encrypted_data.len(), skip);
        return None;
    }
    ...
    let decrypted = match cipher.decrypt_padded_mut::<cbc::cipher::block_padding::NoPadding>(&mut buf)
    {
        Ok(d) => d,
        Err(e) => {
    ...
    // Clean: remove null bytes and non-printable characters

...
fn score_match(
    ...
    cand_author: &str,
    cand_length: u64,
    cfg: &crate::config::server::MirrorsConfig,
) -> f64 {
...
pub async fn resolve_with_mirrors(
    ...
    if mirrors.scoring {
        resolve_scored(manager, track_info, identifier, mirrors, routeplanner).await
    } else {
        resolve_first(manager, track_info, identifier, mirrors, routeplanner).await
    }
}

// ── Scoring disabled:
/// Try each provider in order and return the very first resolvable track.
async fn resolve_first(
    ...
    let query = build_query(track_info);
    let original_source_name = source_name_for(manager, identifier);

    for provider in &mirrors.providers {
        let resolved = match expand_provider(provider, isrc, &query) {
            Some(r) => r,
            None => {
        ...
        if should_skip(&resolved, manager, original_source_name.as_deref()) {
            continue;
        }
        ...
        if let Some(track) = res {
            tracing::debug!("[Mirror] first-match accepted via '{}'", resolved);
            return Some(track);
        }
    }
...
// ── Scoring enabled: parallel-race + sequential fallback 
/// 1. Split providers into **free** (parallel race) and **throttled** (sequential,
///    last resort) based on `mirrors.throttled_prefixes`.
/// 2. Race free providers via `FuturesUnordered`; return immediately on
///    `immediate_use` score, otherwise collect the global best.
/// 3. If no free result clears `immediate_use`, try throttled providers
///    sequentially, accepting any score above `min_similarity`.
/// 4. Last resort: return the best free-phase result even if below `immediate_use`.
async fn resolve_scored(
    ...
    if !free_providers.is_empty() {
        let mut futs: FuturesUnordered<_> = free_providers
            .iter()
            .map(|p| search_provider(manager, track_info, p, routeplanner.clone(), mirrors, false))
            .collect();
        ...
        while let Some(result) = futs.next().await {
            if let Some(mr) = result {
                ...
                if mr.score >= mirrors.immediate_use {
        ...
        if global_best
            .as_ref()
            .is_some_and(|b| b.score >= mirrors.immediate_use)
        {
    ...
    for provider in &throttled_providers {
        if let Some(mr) =
            search_provider(manager, track_info, provider, routeplanner.clone(), mirrors, true)
                .await
        {
            tracing::info!(
...
async fn search_provider(
    ...
    for (score, info) in scored.into_iter().take(limit) {
        ...
        if let Some(track) = resolve_nested_track(manager, id, routeplanner.clone()).await {
...
fn build_query(track_info: &crate::protocol::tracks::TrackInfo) -> String {
    if !track_info.author.is_empty() && track_info.author != "unknown" {
        format!("{} {}", track_info.title, track_info.author)
    } else {
...
fn expand_provider(provider: &str, isrc: &str, query: &str) -> Option<String> {
    if isrc.is_empty() && provider.contains("%ISRC%") {
        return None;
    }
    Some(provider.replace("%ISRC%", isrc).replace("%QUERY%", query))
}


fn source_name_for(manager: &SourceManager, identifier: &str) -> Option<String> {
    manager
        .sources
        .iter()
...
fn should_skip(resolved: &str, manager: &SourceManager, original_source_name: Option<&str>) -> bool {
    if let Some(src) = manager.sources.iter().find(|s| s.can_handle(resolved)) {
        ...
        if Some(src.name()) == original_source_name {
            tracing::debug!(

...
use crate::sources::http::HttpTrack;
        ...
        std::thread::spawn(move || {
            ...
            runtime.block_on(async {
                let url = match resolve_media_url(&client, &token_tracker, &info.identifier).await {
                    ...
                    Ok(None) => None,
                    Err(e) => {
                        error!("Qobuz: Failed to resolve media URL for {}: {}", info.identifier, e);
                        None
                    }
                };
...
async fn resolve_media_url(client: &reqwest::Client, token_tracker: &QobuzTokenTracker, track_id: &str) -> crate::common::types::AnyResult<Option<String>> {
    let tokens = token_tracker.get_tokens().await
    ...
    let sig_data = format!("trackgetFileUrlformat_id{}intent{}track_id{}{}{}", format_id, intent, track_id, unix_ts, tokens.app_secret);
    ...
    if let Some(url) = json.get("url").and_then(|v| v.as_str()) {
        if let Some(is_sample) = json.get("sample").and_then(|v| v.as_bool()).or_else(|| json.get("sample").and_then(|v| v.as_str()).map(|s| s == "true")) {
            if is_sample {
                return Ok(None);
            }
        }

...
use crate::{
    ...
    sources::{
        SourcePlugin, StreamInfo,
        plugin::BoxedTrack,
        qobuz::{token::QobuzTokenTracker, track as qobuz_track, track::QobuzTrack},
    },
};
...
impl SourcePlugin for QobuzSource {
    ...
    async fn get_stream_url(&self, identifier: &str, _itag: Option<i64>) -> Option<StreamInfo> {
        ...
        let url = qobuz_track::resolve_media_url(&self.client, &self.token_tracker, &id)
            .await
            .ok()
            .flatten()?;

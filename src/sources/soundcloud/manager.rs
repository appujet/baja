...
impl SoundCloudSource {
  ...
  async fn load_user(&self, url: &str) -> LoadResult {
    ...
    match sub_path {
      ...
      "albums" => {
        self
          .load_collection_tracks(user_id, &user_name, "albums", "Albums from", &client_id)
          .await
      }
      "sets" => {
        self
          .load_collection_tracks(user_id, &user_name, "playlists", "Sets from", &client_id)
          .await
      }
      "reposts" => {
        self
          .load_collection_tracks(user_id, &user_name, "reposts", "Reposts from", &client_id)
          .await
      }
      "tracks" => {
        self
          .load_collection_tracks(user_id, &user_name, "tracks", "Tracks from", &client_id)
          .await
      }
      "" | "spotlight" => {

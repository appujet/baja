...
impl AppleMusicSource {
  ...
  pub(crate) async fn resolve_album(&self, id: &str) -> LoadResult {
    ...
    if next_url.is_some() && (self.album_load_limit == 0 || self.album_load_limit > 1) {
      let extra = self.fetch_paginated_tracks(next_url, self.album_load_limit, self.album_page_load_concurrency).await;
  ...
  pub(crate) async fn resolve_playlist(&self, id: &str) -> LoadResult {
    ...
    if next_url.is_some() && (self.playlist_load_limit == 0 || self.playlist_load_limit > 1) {
      let extra = self.fetch_paginated_tracks(next_url, self.playlist_load_limit, self.playlist_page_load_concurrency).await;
      all_items.extend(extra);
    }
  ...
  async fn fetch_paginated_tracks(
    ...
    if initial_next.contains("offset=") {
      let base_url = initial_next.split("offset=").next().unwrap_or(&initial_next).to_string();
    ...
    while let Some(url) = next {
      ...
      next = data.get("next").and_then(|v| v.as_str()).map(|s| s.to_string());
      pages_fetched += 1;
    }

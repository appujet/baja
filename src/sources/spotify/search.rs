use std::sync::Arc;
use serde_json::json;
use crate::api::tracks::{PlaylistData, PlaylistInfo, SearchResult, Track};
...
impl SpotifySearch {
  pub async fn get_autocomplete(
    ...
    if let Some(items) = data.pointer("/albums/items").and_then(|v| v.as_array()) {
      for item in items {
        ...
        let url = item.get("external_urls").and_then(|v| v.get("spotify")).and_then(|v| v.as_str());
        
        if url.is_none() || name == "Unknown Album" {
          continue;
        }

        let artwork = item.get("images").and_then(|v| v.get(0)).and_then(|v| v.get("url")).and_then(|v| v.as_str());
        let author = item.get("artists").and_then(|v| v.get(0)).and_then(|v| v.get("name")).and_then(|v| v.as_str()).unwrap_or("Unknown Artist");
    ...
    if let Some(items) = data.pointer("/artists/items").and_then(|v| v.as_array()) {
      for item in items {
        let name = item.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown Artist");
        let url = item.get("external_urls").and_then(|v| v.get("spotify")).and_then(|v| v.as_str());
    ...
    if let Some(items) = data.pointer("/playlists/items").and_then(|v| v.as_array()) {
      for item in items {
        let name = item.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown Playlist");
        let url = item.get("external_urls").and_then(|v| v.get("spotify")).and_then(|v| v.as_str());
        ...
        let artwork = item.get("images").and_then(|v| v.get(0)).and_then(|v| v.get("url")).and_then(|v| v.as_str());
        let author = item.get("owner").and_then(|v| v.get("display_name")).and_then(|v| v.as_str());
  ...
  pub async fn search_full(
    ...
    let data = SpotifyHelpers::partner_api_request(client, token_tracker, "searchDesktop", variables, hash).await?;
    ...
    if all_types || types.contains(&"album".to_string()) {
      ...
      {
        for item in items {
          if let Some(album_data) = item.get("item").or_else(|| item.get("itemV2")).and_then(|v| v.get("data")) {
            let name = album_data.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown Album");
            let uri = album_data.get("uri").and_then(|v| v.as_str()).unwrap_or("");
            let id = uri.split(':').last().unwrap_or("");
            let artwork = album_data.pointer("/coverArt/sources/0/url").and_then(|v| v.as_str());
            let author = album_data.pointer("/artists/items/0/profile/name").and_then(|v| v.as_str()).unwrap_or("Unknown Artist");
    ...
    if all_types || types.contains(&"artist".to_string()) {
      ...
      {
        for item in items {
          if let Some(artist_data) = item.get("item").or_else(|| item.get("itemV2")).and_then(|v| v.get("data")) {
            let name = artist_data.pointer("/profile/name").and_then(|v| v.as_str()).unwrap_or("Unknown Artist");
            let uri = artist_data.get("uri").and_then(|v| v.as_str()).unwrap_or("");
            let id = uri.split(':').last().unwrap_or("");
            let artwork = artist_data.pointer("/visuals/avatar/sources/0/url").and_then(|v| v.as_str());
    ...
    if all_types || types.contains(&"playlist".to_string()) {
      ...
      {
        for item in items {
          if let Some(playlist_data) = item.get("item").or_else(|| item.get("itemV2")).and_then(|v| v.get("data")) {
            let name = playlist_data.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown Playlist");
            let uri = playlist_data.get("uri").and_then(|v| v.as_str()).unwrap_or("");
            let id = uri.split(':').last().unwrap_or("");
            let artwork = playlist_data.pointer("/images/items/0/sources/0/url").and_then(|v| v.as_str());
            let author = playlist_data.pointer("/ownerV2/name").and_then(|v| v.as_str());

...
pub fn parse_search_playlist(json: &Value, type_: &str) -> Option<PlaylistData> {
  ...
  if url.is_empty() {
    if let Some(id) = json.get("id").and_then(|v| {
      ...
      if id.starts_with("/") || id.starts_with("http") {
        ...
        let slug = title.to_lowercase().replace(|c: char| !c.is_alphanumeric() && c != ' ', "").replace(' ', "-");
  ...
  let artwork_url = json.get("image").and_then(|v| v.as_str()).map(|s| {
  ...
  let total_tracks = json
    ...
    .or_else(|| {
      json.pointer("/more_info/song_pids").and_then(|v| v.as_str()).map(|s| {
        if s.is_empty() {
          0
        } else {
  ...
  if author.is_none() {
    let first = json.pointer("/more_info/firstname").and_then(|v| v.as_str()).filter(|s| !s.is_empty());
    let last = json.pointer("/more_info/lastname").and_then(|v| v.as_str()).filter(|s| !s.is_empty());
    if let Some(f) = first {
  ...
  if author.is_none() {
    author = json
      ...
      .and_then(|v| v.as_str())
      .filter(|s| !s.is_empty())
      .or_else(|| json.get("description").and_then(|v| v.as_str()).filter(|s| !s.is_empty()))
      .map(|s| s.to_string());
  }

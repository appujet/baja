...
use super::track::AudiomackTrack;
use super::utils::build_auth_header;
use crate::{
...
impl AudiomackSource {
    pub fn new(config: Option<crate::configs::AudiomackConfig>) -> Self {
        ...
        Self {
            client,
            song_regex: Regex::new(r"https?://(?:www\.)?audiomack\.com/(?P<artist>[^/]+)/song/(?P<slug>[^/?#]+)").unwrap(),
            album_regex: Regex::new(r"https?://(?:www\.)?audiomack\.com/(?P<artist>[^/]+)/album/(?P<slug>[^/?#]+)").unwrap(),
            playlist_regex: Regex::new(r"https?://(?:www\.)?audiomack\.com/(?P<artist>[^/]+)/playlist/(?P<slug>[^/?#]+)").unwrap(),
            artist_regex: Regex::new(r"https?://(?:www\.)?audiomack\.com/(?P<artist>[^/?#]+)(?:/songs)?/?$").unwrap(),
            likes_regex: Regex::new(r"https?://(?:www\.)?audiomack\.com/(?P<artist>[^/]+)/likes").unwrap(),
    ...
    async fn make_request(&self, method: reqwest::Method, endpoint: &str, query_params: Option<BTreeMap<String, String>>) -> Option<Value> {
        let url = format!("{}{}", API_BASE, endpoint);
        tracing::debug!("Audiomack request: {} {} params: {:?}", method, url, query_params);
        ...
        if !status.is_success() {
            warn!("Audiomack API error status: {} for endpoint: {}", status, endpoint);
            return None;
        }
    ...
    fn parse_track(&self, json: &Value) -> Option<Track> {
        ...
        let duration_sec = if let Some(d) = json.get("duration").and_then(|v| v.as_u64()) {
            ...
            d as u64
        } else if let Some(d) = json.get("duration").and_then(|v| v.as_str()).and_then(|s| s.parse::<u64>().ok()) {
            d
        } else {
            0
        };
        ...
        let uploader_slug = json.pointer("/uploader/url_slug")
            .and_then(|v| v.as_str())
        ...
        let uri = Some(format!("https://audiomack.com/{}/song/{}", uploader_slug, url_slug));

        let artwork_url = json.get("image")
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        let isrc = json.get("isrc")
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
    ...
    async fn search(&self, query: &str) -> LoadResult {
        ...
        if let Some(json) = self.make_request(reqwest::Method::GET, "/search", Some(params)).await {
    ...
    async fn get_song(&self, artist: &str, slug: &str) -> LoadResult {
        ...
        if let Some(json) = self.make_request(reqwest::Method::GET, &endpoint, None).await {
    ...
    async fn get_playlist_items(&self, type_: &str, artist: &str, slug: &str) -> LoadResult {
        ...
        if let Some(json) = self.make_request(reqwest::Method::GET, &endpoint, None).await {
            if let Some(results) = json.get("results") {
                let name = results.get("title").and_then(|v| v.as_str()).unwrap_or("Unknown").to_string();
                let tracks_val = results.get("tracks");
                
                if tracks_val.is_none() {
                    tracing::debug!("Audiomack {} results missing tracks field: {:?}", type_, results);
                }

                let tracks: Vec<Track> = tracks_val
                    .and_then(|v| v.as_array())
                    .map(|arr| arr.iter().filter_map(|item| self.parse_track(item)).collect())
                    .unwrap_or_default();
                ...
                tracing::debug!("Audiomack {} response missing results field: {:?}", type_, json);
            }
        }
        LoadResult::Empty {}
    }
    ...
    async fn get_artist(&self, artist_slug: &str) -> LoadResult {
        if let Some(json) = self.make_request(reqwest::Method::GET, &format!("/artist/{}", artist_slug), None).await {
            let Some(results) = json.get("results") else { return LoadResult::Empty {}; };
            ...
            let Some(name) = results.get("name").and_then(|v| v.as_str()) else { return LoadResult::Empty {}; };
            ...
            if let Some(tracks_json) = self.make_request(reqwest::Method::GET, "/search_artist_content", Some(params)).await {
                if let Some(track_results) = tracks_json.get("results").and_then(|v| v.as_array()) {
                    let tracks: Vec<Track> = track_results.iter()
                        .filter_map(|item| self.parse_track(item))
                        .collect();
    ...
    async fn get_artist_likes(&self, artist_slug: &str) -> LoadResult {
        if let Some(json) = self.make_request(reqwest::Method::GET, &format!("/artist/{}", artist_slug), None).await {
            ...
            if let Some(likes_json) = self.make_request(reqwest::Method::GET, &format!("/artist/{}/favorites", artist_slug), None).await {
                if let Some(like_results) = likes_json.get("results").and_then(|v| v.as_array()) {
                    let tracks: Vec<Track> = like_results.iter()
                        .filter_map(|item| self.parse_track(item))
                        .collect();
...
impl SourcePlugin for AudiomackSource {
    ...
    async fn get_track(&self, identifier: &str, routeplanner: Option<Arc<dyn crate::routeplanner::RoutePlanner>>) -> Option<Box<dyn PlayableTrack>> {

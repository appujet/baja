  ...
  pub async fn initialize_access_token(self: std::sync::Arc<Self>) {
    ...
    match self.fetch_device_code().await {
      Ok(response) => {
        ...
        let s1_padding = inner_width.saturating_sub(s1_prefix.len() + verification_url.len());
        crate::log_println!(
        ...
        Ok(response) => {
          if let Some(error) = response["error"].as_str() {
            match error {
              ...
              "slow_down" => {
                interval_timer =
                  tokio::time::interval(std::time::Duration::from_secs(interval + 5));
                continue;
              }
              "expired_token" => {
                tracing::error!(

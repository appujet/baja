...
use super::{
    ...
    http::HttpSource,
    jiosaavn::{JioSaavnSource, JioSaavnStreamContext},
...
pub struct SourceManager {
    ...
    pub jiosaavn_stream_ctx: Option<Arc<JioSaavnStreamContext>>,
...
impl SourceManager {
    ...
    pub fn new(config: &crate::configs::Config) -> Self {
        ...
        let mut jiosaavn_stream_ctx = None;
        ...
        if config.sources.jiosaavn {
            match JioSaavnSource::new(
                config.jiosaavn.clone(),
                http_pool.get(jiosaavn_proxy.clone()),
            ) {
                Ok(src) => {
                    tracing::info!("Loaded source: JioSaavn");
                    jiosaavn_stream_ctx = Some(src.stream_context());
                    sources.push(Box::new(src));
                }
                Err(e) => {
                    tracing::error!("JioSaavn source failed to initialize: {}", e);
                }
            }
        }

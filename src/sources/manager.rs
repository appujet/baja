...
pub struct SourceManager {
    ...
    pub deezer: Option<Arc<DeezerSource>>,
...
impl SourceManager {
    ...
    pub fn new(config: &crate::configs::Config) -> Self {
        ...
        let mut youtube: Option<Arc<YouTubeSource>> = None;
        let mut deezer: Option<Arc<DeezerSource>> = None;
        ...
        if config.sources.youtube {
            ...
            // Create one instance for the sources vec (requires Box<dyn SourcePlugin>)
            let yt_box = YouTubeSource::new(config.youtube.clone(), yt_client.clone());
            youtube_cipher_manager = Some(yt_box.cipher_manager());
            sources.push(Box::new(yt_box));
            // Create a second Arc'd instance for direct get_stream_info access
            // Both instances share the same underlying http client and config.
            let yt_arc = Arc::new(YouTubeSource::new(config.youtube.clone(), yt_client));
            youtube = Some(yt_arc);
        }
        ...
        if config.sources.deezer && deezer_token_provided && deezer_key_provided {
            ...
            let dz_arc = Arc::new(DeezerSource::new(
                config.deezer.clone().unwrap_or_default(),
                http_pool.get(deezer_proxy.clone()),
            ).unwrap()); 
            deezer = Some(dz_arc);
            // One for the generic plugin list
            let dz_plugin = DeezerSource::new(
                config.deezer.clone().unwrap_or_default(),
                http_pool.get(deezer_proxy.clone()),
            ).unwrap();
            sources.push(Box::new(dz_plugin));
        }

...
impl VoiceGateway {
    ...
    pub async fn run(self) -> AnyResult<()> {
        ...
        loop {
            ...
            match outcome {
                ...
                Ok(SessionOutcome::ReconnectFresh) => {
                    // Successful session — reset the backoff counter so subsequent
                    // disconnects start the delay from the beginning again.
                    backoff.reset();
                    is_resume = false;
                    seq_ack.store(-1, Ordering::Relaxed);
                    let delay = backoff.next();
                    debug!("[{}] Reconnecting fresh in {:?}", self.guild_id, delay);
                    tokio::time::sleep(delay).await;
                }
    ...
    async fn connect(&self, is_resume: bool, seq_ack: Arc<AtomicI64>) -> AnyResult<SessionOutcome> {
        ...
        let connect_cancel = self.cancel_token.child_token();
        let guild_id = self.guild_id.clone();
        let write_cancel = connect_cancel.clone();
                ...
                tokio::select! {
                    ...
                    msg = rx.recv() => {
                        ...
                        if let Err(e) = write.send(msg).await {
                            debug!("[{}] WS write error: {}", guild_id, e);
                            break;
                        }
                    }
                }
            ...
            tokio::select! {
                ...
                msg = read.next() => {
                    let msg = match msg {
                        Some(Ok(msg)) => msg,
                        Some(Err(e)) => {
                            debug!("[{}] WS read error: {}", self.guild_id, e);
                    ...
                    match msg {
                        ...
                        Message::Close(frame) => {
                            ...
                            // Any unrecognised close code: reconnect fresh (no resume).
                            break SessionOutcome::ReconnectFresh;
                        }
                        _ => {}
                    }
                }

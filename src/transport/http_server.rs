...
use crate::{
    ...
    transport::{
        ...
        routes::{lyrics, player, sources, stats},
    },
};
const API_V4: &str = "/v4";
...
pub fn router(state: Arc<AppState>) -> Router {
    ...
    Router::new()
        ...
        .route("/youtube", get(sources::youtube::get_youtube_info))
        .route("/youtube/stream/{video_id}", get(sources::youtube::youtube_stream))
        .route("/youtube/oauth/{refresh_token}", get(sources::youtube::youtube_oauth_refresh))
        .route("/jiosaavn/stream/{track_id}", get(sources::jiosaavn::jiosaavn_stream))
